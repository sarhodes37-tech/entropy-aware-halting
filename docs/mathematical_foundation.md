...
The OptimizedEntropyGate operates like a thermodynamic sensor for Large Language Model (LLM) outputs. Rather than parsing semantic meaning (which incurs high latency), it evaluates the statistical confidence of the model's token distribution in real-time.
...
To successfully intercept sustained N-gram loops (e.g., repeating the same word infinitely), the current token's Z-score *must* be evaluated against the previous state of the baseline. If the new baseline is calculated before the evaluation, the rolling mean immediately absorbs the flatlined entropy of the loop, the variance drops to zero, and the anomaly becomes invisible.
 ...
 * **Preserves Computational Efficiency:** The floor requires a single O(1) max() operation. This maintains the ultra-low overhead (~0.0075 ms) required to act as a lightweight, first-pass filter before the execution reaches heavier regex evaluations.