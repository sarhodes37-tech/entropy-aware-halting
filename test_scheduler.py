...
# =====================================================================
# Step Execution & State Directive Tests
# =====================================================================
...
def test_scheduler_step_eta_calculation():
    ...
    """
    scheduler = EntropyAwareScheduler()
    # Step 0: Establish high baseline entropy ([0.5, 0.5] -> H = 1.0)
    scheduler.step(torch.tensor([0.5, 0.5]), cost=10, state="s1")

    # Step 1: H drops to 0.0 (delta_h = 1.0). cost = 5.
...
def test_scheduler_step_negative_yield():
    ...
    across the configured negative_yield_window.
    """
    ...
    assert res1.directive == "CONTINUE"  # Requires 2 consecutive negative yield steps
...
def test_scheduler_step_optimal_convergence():
    """
    Validates OPTIMAL_CONVERGENCE directive when entropy falls below confidence_threshold
    and utility gain drops below utility_epsilon.
    """
    scheduler = EntropyAwareScheduler(
    ...
    # Step 0: Establish high baseline entropy
    scheduler.step(torch.tensor([0.5, 0.5]), cost=0, state="s1")
    
    # Step 1: Drop entropy to 0 to trigger initial high utility jump
    scheduler.step(torch.tensor([1.0, 0.0]), cost=0, state="s2")
    
    # Step 2: Maintain low entropy so utility_gain < utility_epsilon
...
def test_scheduler_step_irreducible_uncertainty():
    """
    Validates IRREDUCIBLE_UNCERTAINTY directive when entropy delta remains below 
    entropy_delta_threshold across the stagnation_window.
    """
    scheduler = EntropyAwareScheduler(stagnation_window=2, entropy_delta_threshold=0.01)

    # Step 0: High baseline entropy
    scheduler.step(torch.tensor([0.5, 0.5]), cost=0, state="s1")
    
    # Step 1: Stagnant delta (delta_h ~ 0)
    res1 = scheduler.step(torch.tensor([0.5, 0.5]), cost=0, state="s2")
    assert res1.directive == "CONTINUE"

    # Step 2: Second stagnant step (hits stagnation_window=2)
